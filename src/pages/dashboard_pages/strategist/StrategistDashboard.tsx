...
export const StrategistDashboard: React.FC = () => {
  ...
  return (
    <div className="w-full pb-28 md:pb-0">
      ...
      <div className="mb-4">
        <MetricCard metric={strategistMetrics[0]} />
      </div>

      <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {strategistMetrics.slice(1, 4).map((metric) => (
          <MetricCard key={metric.id} metric={metric} />
        ))}
      </div>

      <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {strategistMetrics.slice(4).map((metric) => (
          <MetricCard key={metric.id} metric={metric} />
        ))}
      </div>
      ...
      <div className="rounded-2xl border border-gray-900 bg-[#0D0B07] p-4 sm:p-6">
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-white">Recent song submissions</h3>
        ...
        <SongSubmissionsTable rows={songSubmissions.slice(0, 2)} />
      </div>
    </div>
  );
};
